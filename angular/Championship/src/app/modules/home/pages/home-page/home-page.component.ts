...
// Sort by Drivers by Name in order to manipulate same rows
  this.races.forEach (r=>{ r.RaceDriver.sort((a,b)=>{
  ...
  this.races.forEach (r=>{
    ...
    console.log ("Para la carrera:"+r.name);
    r.RaceDriver.forEach(d=>{
      console.log ( d.name+" t:"+d.time+" lugar:"+(d.position||-1));
    });
